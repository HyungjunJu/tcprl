...
from deprecated.a3c_actor import Global_Actor, Worker_Actor
from deprecated.a3c_critic import Global_Critic, Worker_Critic
from deprecated.a3c_etc import *
...
action_dimension = 1 #change cwnd
...
def throughput_calc(cwnd, rtt):
    avg_tp = 3/4 * cwnd / rtt
    return avg_tp


def utility_function(state, lossTrace):
    cwnd = state[5]
    rtt = state[9]
    avg_tp = throughput_calc(cwnd, rtt)
    if avg_tp == 0:
        avg_tp = 1
...
def state_trans_ewma_ignore(state, cwnd_ewma, rtt_ewma, losstrace):
    rtt = state[9]
    rtt_min = state[10]
    rtt_diff = rtt - rtt_min
    rtt_e = rtt_ewma.get_current(rtt_diff)
    cwnd = state[5]
    cwnd_e = cwnd_ewma.get_current(cwnd)
    loss = losstrace.get_error_prob()
    output = [rtt_e, cwnd_e, loss]
    return output

def state_trans(state, cwnd_ewma, rtt_ewma, losstrace):
    rtt = state[9]
    rtt_min = state[10]
    rtt_diff = rtt - rtt_min
...
def get_reward(state, losstrace, U_old):
    u = utility_function(state, lossTrace=losstrace)
    u_diff = u - U_old
    if u_diff > 0:
        return 1, u
    else:
    ...
    def run(self):
        ...
        while global_episode_count <= int(self.max_episode_num):
            ...
            while not done:
                ...
                state_tr = state_trans_ewma_ignore(state, cwnd_ewma=cwnd_ewma, rtt_ewma=rtt_ewma, losstrace=losstrace)
                ...
                next_state_tr = state_trans(next_state, cwnd_ewma=cwnd_ewma, rtt_ewma=rtt_ewma, losstrace=losstrace)
                ...
                #batch_reward.append((reward+8)/8) # train reward boundary clipping need
