...
def extract_state(state, rtt_ewma, losstrace):
    rtt = state[9]
    rtt_min = state[10]
...
for e in range(total_episodes):
    state = env.reset()
    cwnd = state[5]
    state_trans = extract_state(state, rtt_ewma, lt_1)
    ...
    for time in range(max_env_steps):
        ...
        if done:
            ...
            break
        next_state_trans = extract_state(next_state, rtt_ewma, lt_1)
