...
int main (int argc, char *argv[])
{
  ...
  double error_p = 0.0;
  std::string bottleneck_bandwidth = "2Mbps"; //default 2Mbps
  std::string bottleneck_delay = "0.01ms"; // default 0.01 ms
  std::string access_bandwidth = "10Mbps"; // default 10Mbps
  std::string access_delay = "20ms"; //default 20ms
  std::string prefix_file_name = "TcpVariantsComparison";
  std::string compareTcpproto = "TcpNewReno";
  uint64_t data_mbytes = 1; //1mb
  uint32_t mtu_bytes = 576;
  ...
  compareTcpproto = std::string ("ns3::") + compareTcpproto;
  ...
  for (uint32_t i = 0; i < d.LeftCount (); ++i)
  {
    if (i > 0)
    {/*
      Ptr<Node> node = d.GetLeft(i);
      TypeId tcpTid;
      tcpTid = TypeId::LookupByName("ns3::TcpNewReno");
      std::stringstream nodeId;
      nodeId << node->GetId();
      std::string nodeIdString = "/NodeList/" + nodeId.str() +"/$ns3::TcpL4Protocol/SocketType";
      Config::Set(nodeIdString, TypeIdValue(tcpTid));
      */
      TypeId tcpTid;
      NS_ABORT_MSG_UNLESS (TypeId::LookupByNameFailSafe (compareTcpproto, &tcpTid), "TypeId " << compareTcpproto << " not found");
      Config::SetDefault ("ns3::TcpL4Protocol::SocketType", TypeIdValue (TypeId::LookupByName (compareTcpproto)));
    }
