...
for e in range(total_episodes):
    state = env.reset()
    max_cwnd = env.action_space.high[0]
    print(max_cwnd)
    cwnd = state[5]
    state_trans = extract_state(state, cwnd_ewma, rtt_ewma, lt_1)
    state = np.reshape(state, [1, state_space])
    state_trans = normalize_state(state_trans, state, max_cwnd)
    ...
    for time in range(max_env_steps):
        ...
        next_state_trans = normalize_state(next_state_trans, next_state, max_cwnd)
...
print(total_reward)
# save current model
model.save_weights("./save_weights/_model.h5")